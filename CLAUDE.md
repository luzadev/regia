...
- **Tracking per poltrona**: `stations[].tracking` = `"normal"` (segue l'ospite), `"upper"` (mezzo busto), `"closeup"` (primo piano); assente = inquadratura fissa, che resta il default. Si sceglie dalla regia in anteprima; **in onda solo con conferma esplicita** (`on_air: true`, doppio tocco in dashboard), per l'ospite che deve alzarsi o spostarsi: il cambio si vede sul mixer. Frecce e zoom restano solo in anteprima. Con il tracking attivo i comandi manuali e il salvataggio dell'inquadratura sono rifiutati (`tracking_on`): l'SDK spegnerebbe il tracking al primo movimento manuale. Quando la telecamera torna online riceve il suo tracking invece dell'inquadratura fissa.
...
control → server : { "type": "camera_tracking", "station": "post-03", "mode": "off"|"normal"|"upper"|"closeup", "on_air": true }  // on_air solo se confermato
